...
function vpsmanage_onload(){
	...
	$('#vps-desc-hostname').attr('data-content', '');
	...
	$('#vnc, #vnc-cell, #vncpass-cell, #control-panel-tab, #rescue-mode-cell, #backups-cell, #ipv6_subnets-cell, #monitoring-tab, #proc-cell, #services, #alerts-cell, #console-cell, #recipe-mode-cell, #os-cell, #ssh, #ssh-cell, #hostname-tab, #change-password-tab, #ips-cell, #self_shutdown-cell, #managevdf-cell, #sshkeys-cell, #rdns-cell').hide();
...
function show_sshkeys_window() {
	...
	AJAX('[[API]]act=sshkeys&svs='+N['vpsid'], function(data) {
		...
		if(empty(data['ssh_keys'])) {
			...
			$('#sshkey_list').html('<div class="notice"><img src="[[images]]notice.png" /> &nbsp; {{sshkey_no_keys_acct_whmcs}}</div>');
			return false;
		}
...
function show_osreinstall_window(){
	...
	var os_badge = '<div class="col-12 col-sm-6 col-md-4 mb-2"><div class="os_badge media"><i class="fa fa-2x fa-caret-down" aria-hidden="true"></i><div class="media-left p-1"><img class="distro_img media-object" src=""></div><div class="media-body p-2"><h4 class="distro_name media-heading"></h4><span class="version"></span></div><div class="os_badge_list"></div></div></div>';
...
function set_backups_table(backups_list, restore) {
	...
	for(var i=0; i<backups_list.length; i++) {
		...
		var action_restore = (restore ? '<a href="#" title="{{bkup2_restore}}" onclick="restore(' + year + mon + day + '); return false;"><i class="fas fa-history restore-icon text-secondary mr-2"></i>' : '');
		
		table_data.push([year + "-" + mon + "-" + day, action_restore + '<a href="#" title="{{delete}}" onclick="backup_delete(' + year + mon + day + '); return false;"><i class="fa fa-1x fa-times delete" aria-hidden="true"></i></a>']);
	}
...
function show_monitor_window(open_window, mon){
	...
	AJAX('[[API]]act=monitor&svs=' + N['vpsid'] + '&show=' + mon, function(data) {
		...
		$('#cpu_show_graph, #ram_show_graph, #diskgraphcell, #inodesgraphcell').css('display', 'block');
...
function fill_ostemplates(prefix){
	...
	tmp_mgs = merge_array(user_mgs, mgs_array);
	...
	if(virt == "openvz" || virt == "proxo" || virt == "vzo"){
		...
		$("#"+prefix+"tr_swap").hide();		
	}else if(virt == "lxc" || virt == "proxl" || virt == "proxk" || virt == "kvm" || virt == "vzk" || virt == "xcp"){
