...
function virtualizor_TerminateAccount($params) {
	...
	if($ctrlpanel != -1 && $ctrlpanel != 'none'){
		...
		if($ctrlpanel == 'cpanel' && !empty($virtualizor_conf['cp']['buy_cpanel_login']) && !empty($virtualizor_conf['cp']['buy_cpanel_apikey'])){
			...
			$data = Virtualizor_Curl::make_api_call($params["serverip"], $params["serverpassword"], 'index.php?act=vs&vpsid='.$params['customfields']['vpsid']);
...
function virtualizor_suspend_net($params) {
	...
	$action = 'act=vs&suspend_net='.$params['customfields']['vpsid'];
...
function virtualizor_unsuspend_net($params) {
	...
	$action = 'act=vs&unsuspend_net='.$params['customfields']['vpsid'];
...
function virtualizor_enduser_panel($vars){
	...
	$redirect_url = 'https://'.$tmp_hostname.':4083/'.$ret['token_key'].'/?as='.$ret['sid'].'&goto_cp='.rawurlencode(virtualizor_get_current_url()).'&svs='.$sel_res['value'];
